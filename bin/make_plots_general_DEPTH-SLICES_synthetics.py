...
def read_gmt_synth_blocks(filepath):

    # parsing
    with open(filepath, "r") as IN:
        list_array = []
        new_arr = []  # initialize new array for the next block
        ...
        for xx, line in enumerate(IN):
            ...
            if line[0] == ">":
                # close array and append to list
                if len(new_arr) != 0:
                    list_array.append(np.array(new_arr))
                    new_arr = []  # initialize new array for the next block
                continue

            else:
                new_arr.append((float(line_fields[0]), float(line_fields[1])))

        # End of file, append last array
        if len(new_arr) != 0:
            list_array.append(np.array(new_arr))
    #
    return list_array
...
for _dep in np.unique(np.sort(grid.coordinates[:, -1])):
    ...
    # ---------------------- SynthBoundaries
    if configs["SYNTHETICS"]["plot_contours"]:
        _synth_file_name = Path("%s/SYNTH.-10_contour.%.1f.txt" % (
                                configs["SYNTHETICS"]["folder"], _dep))

        # Low Velocity perturbations
        if _synth_file_name.exists():
            _synth_blocks = read_gmt_synth_blocks(str(_synth_file_name))
        else:
            print("Cannot find file:  %s" % str(_synth_file_name))
            _synth_blocks = None

        if _synth_blocks:
            for nodes in _synth_blocks:
                ax.plot(nodes[:, 0], nodes[:, 1], color="darkred",
                        linestyle='dashed', lw=1.5)  # alpha=0.25)

        # Higher Velocity perturbations
        _synth_file_name = Path("%s/SYNTH.+10_contour.%.1f.txt" % (
                                configs["SYNTHETICS"]["folder"], _dep))
        ...
        if _synth_blocks:
            for nodes in _synth_blocks:
                ax.plot(nodes[:, 0], nodes[:, 1], color="darkblue",
                        linestyle='dashed', lw=1.5)  # alpha=0.25)
